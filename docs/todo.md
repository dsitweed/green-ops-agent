...
- [ ] Tạo thư mục Terraform ở root, ví dụ `terraform/`, có provider AWS và biến `region`.
- [ ] Tạo một EC2 instance nhỏ, dùng AMI hợp lệ theo region, gắn security group tối thiểu và key/access method phù hợp.
- [ ] Gắn các tag bắt buộc: `Name`, `Environment`, `Owner`, `ManagedBy=terraform`, `Workload`.
- [ ] Tạo IAM instance profile tối thiểu nếu cần gửi custom metrics hoặc log lên CloudWatch.
- [ ] Tạo CloudWatch log group và metric alarms tối thiểu cho EC2 nếu use case cần.
- [ ] Tách state/backend thử nghiệm khỏi production; không commit secret, state nhạy cảm hoặc key pair.
- [ ] Chạy `terraform fmt`, `terraform validate`, `terraform plan` và ghi lại output baseline.
